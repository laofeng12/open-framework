...
import org.ljdp.common.file.POIExcelUtil;
import org.ljdp.common.file.POIResultFile;
...
public abstract class FileBatchTask extends BaseBatchTask {
    ...
    private void doProcessExcelXFile() throws Exception{
...
//                int lastColIndex = sheet.getRow(0).getLastCellNum()+1;//最后一列开始存放日志
    			while( currentReadRow <= sheet.getLastRowNum() && !isCompleted() && isRunning() ) {
    				int currentBatchNum = 0;
    				Row[] records = new Row[batchSize];
    				for(;currentBatchNum < batchSize && currentReadRow <= sheet.getLastRowNum(); ++currentReadRow) {
    					Row row = sheet.getRow(currentReadRow);
    					//不处理空行
    			...
    			if(currentReadRow >= sheet.getLastRowNum()) {
    				currentReadRow = 0;
    			}
    			sheet = null;
            }
