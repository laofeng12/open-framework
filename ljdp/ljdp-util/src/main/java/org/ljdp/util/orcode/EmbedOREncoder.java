...
public class EmbedOREncoder extends BaseZxingCoder{
	// 图片宽度的一般
	public static final int IMAGE_WIDTH = 80;
	public static final int IMAGE_HEIGHT = 80;
	public static final int IMAGE_HALF_WIDTH = IMAGE_WIDTH / 2;
	public static final int FRAME_WIDTH = 2;
	
	public EmbedOREncoder() {
		super();
	}

	public EmbedOREncoder(String character) {
		super(character);
	}
	
	public EmbedOREncoder(Map<EncodeHintType, Object> hints) {
		super();
		this.hints = hints;
	}
	 ...
	 * @throws IOException
	 */
	public String encode(String content, int width, int height, String srcImagePath, String destImagePath,
			String destImageName, String format) throws IOException, WriterException {
		String imageFileName = destImageName + "." + format;
		String imagefullName = FileUtils.joinDirectory(destImagePath, imageFileName);
		File f = new File(FileUtils.joinDirectory(imagefullName));
		if (f.exists()) {
			System.out.println("二维码文件已存在：" + imageFileName);
			return imageFileName;
		}
		ImageIO.write(genBarcode(content, width, height, srcImagePath), format, new File(imagefullName));
		return imageFileName;
	}

	private BufferedImage genBarcode(String content, int width, int height, String srcImagePath)
