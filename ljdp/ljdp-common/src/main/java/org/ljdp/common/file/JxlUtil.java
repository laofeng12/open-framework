...
public class JxlUtil {
	...
	public static WritableCell copyCell(Cell cell, int row) {
		...
		if (type.equals(CellType.NUMBER)) {
			jxl.write.Number num = new Number(cell.getColumn(), row,
					((NumberCell) cell).getValue());
			return num;
		} else if (type.equals(CellType.DATE)) {
			jxl.write.DateTime dt = new DateTime(cell.getColumn(), row,
			...
			jxl.write.Formula formual = new Formula(cell.getColumn(), row, cell
					.getContents());
			return formual;
		} else if(type.equals(CellType.ERROR)) {
			jxl.write.Label label = new Label(cell.getColumn(), row, "");
			return label;
		} else {
			jxl.write.Label label = new Label(cell.getColumn(), row, cell
					.getContents());
			return label;
		}
	}
