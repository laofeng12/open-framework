    ...
    fileList = fileList.filter(file => {
      if (file.response) {
        if (file.response.success) {
          ...
          file.url = `/api/ljdp/attach/memory/view/${d}{file.response.data}`;
        } else {
...
//              open('http://网关:端口/page/filebatch/list.jspx');
              router.push('/batch/batchFileimportTask/list');
            },
          });
        } else {
          Modal.error({
            title: '提交失败',
            content: `失败原因：${d}{response.msg}`,
          });
        }
      },
    });
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title="${resName}-批量导入" action={action}>
