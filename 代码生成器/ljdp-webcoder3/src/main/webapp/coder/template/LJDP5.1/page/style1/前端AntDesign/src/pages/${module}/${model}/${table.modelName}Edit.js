...
@Form.create()
class ${table.modelName}Edit extends PureComponent {
  componentDidMount() {
    const {
      dispatch,
      match: { params },
	...
	dispatch({
        ...
        callback: response => {
          if (response.code === 200) {
        	...
        	message.success(`保存失败：[${d}{response.code}]${d}{response.message}`);
          }
        },
    });
  }
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title="${resName}" content="" action={action}>
        <Card bordered={false}>
...
export default ${table.modelName}Edit;
<#else>
没有选新增，此文件可删除
</#if>