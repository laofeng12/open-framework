...
class ${table.modelName}List extends PureComponent {
  ...
  handleRemove = () => {
    ...
    // if (!selectedRows.lenght) return;
    const ids = selectedRows.map(row => row.${table.keyField}).join(',');
    if(!ids) return;
    const onOkf = () => {
      dispatch({
...
//      }
  ...
  handleImport = () => {
    router.push('/${module}/${table.modelName2}/batch');
  };
  </#if>

  handleShow = (e, key) => {
    router.push(`/${module}/${table.modelName2}/${d}{key}`);
  };

  <#if baseFun.add == "on">
  handleAdd = () => {
    router.push(`/${module}/${table.modelName2}/add/0`);
  };
  
  handleEdit = (e, key) => {
    router.push(`/${module}/${table.modelName2}/edit/${d}{key}`);
  };
  </#if>
