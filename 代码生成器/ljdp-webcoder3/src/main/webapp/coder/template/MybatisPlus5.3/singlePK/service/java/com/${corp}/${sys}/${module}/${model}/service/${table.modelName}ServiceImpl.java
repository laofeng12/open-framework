...
public class ${table.modelName}ServiceImpl extends ServiceImpl<${table.modelName}Mapper, ${table.modelName}> implements ${table.modelName}Service {
	...
	public IPage<${table.modelName}> query(${table.modelName}DBParam params, Pageable pageable){
		...
		IPage<${table.modelName}> page = this.page(
				...
				<#elseif item.useJavaType == "Boolean">
					.isNull(params.get${item.condition?cap_first}_${item.columnName}() != null && params.get${item.condition?cap_first}_${item.columnName}(), "${item.code}")
					.isNotNull(params.get${item.condition?cap_first}_${item.columnName}() != null && !params.get${item.condition?cap_first}_${item.columnName}(), "${item.code}")
				<#elseif item.condition == "nin">
					.notIn(params.get${item.condition?cap_first}_${item.columnName}() != null,"${item.code}", params.get${item.condition?cap_first}_${item.columnName}())
				<#else>
