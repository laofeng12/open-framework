...
public class ${table.modelName}DBParam extends RoDBQueryParam {
	...
	private ${item.useJavaType} ${item.condition}_${item.columnName};//${item.name} ${item.symbol} ?
	</#list>
	...
	<#list dbParamList as item>
	public ${item.useJavaType} get${item.condition?cap_first}_${item.columnName}() {
		return ${item.condition}_${item.columnName};
	}
	public void set${item.condition?cap_first}_${item.columnName}(${item.useJavaType} ${item.columnName}) {
